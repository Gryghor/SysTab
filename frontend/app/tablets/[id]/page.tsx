              ...
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-4">
                ...
                <Button
                  ...
                  onClick={async () => {
                    ...
                    const baseUrl = (api && api.defaults && api.defaults.baseURL)
                      ? api.defaults.baseURL.replace(/\/$/, "")
                      : (process.env.NEXT_PUBLIC_API_URL || "");
                    const url = `${baseUrl}/tablets/${id}/termo-responsabilidade`;
                    const token = localStorage.getItem('token');
                    try {
                      const resp = await fetch(url, { method: 'GET', headers: { Authorization: token ? `Bearer ${token}` : '' } });
                      if (!resp.ok) throw new Error('Falha ao baixar termo');
                      const blob = await resp.blob();
                      const blobUrl = window.URL.createObjectURL(blob);
                      window.open(blobUrl, '_blank');
                    } catch (err) {
                      toast({ title: 'Erro', description: 'Não foi possível gerar o termo.', variant: 'destructive' });
                    }
                  }}
